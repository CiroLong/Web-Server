...
TCPConnection::TCPConnection(std::string IP, int port = 8080)
    : Connection(), ip_(IP), port_(port) {
  if (ip_.empty()) {
    ip_ = defaultIP;
  }
...
TCPConnection::~TCPConnection() { close(server_sock_); }

void TCPConnection::listen() { ::listen(server_sock_, MAX_REQUESTS_HOLD); }
...
TCPHandler *TCPConnection::accept() {
  ...
  int clnt_sock =
      ::accept(server_sock_, (struct sockaddr *)&clnt_addr, &clnt_addr_size);
